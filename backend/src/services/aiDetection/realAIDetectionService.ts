...
export class RealAIDetectionService extends BaseDetectionService {
  ...
  private setupQueueHandlers(): void {
    this.queueService.on(
      'job:process',
      async (job: QueueJob<{ type: string; data: TextAnalysisData | VideoAnalysisData }>) => {
        try {
  ...
  private async processTextAnalysis(
  ...
  private async waitForJobCompletion(jobId: string, timeout: number): Promise<DetectionResult[]> {
    return new Promise((resolve, reject) => {
      ...
      this.queueService.once(`job:completed`, (job: QueueJob<DetectionResult[]>) => {
        if (job.id === jobId) {
          clearTimeout(timer);
          resolve(job.data);
        }
      ...
      this.queueService.once(`job:failed`, (job: QueueJob<DetectionResult[]>) => {
        if (job.id === jobId) {
          clearTimeout(timer);
          reject(job.error);
        }
  ...
  getMetrics() {
    return {
      ...
      averageProcessingTime: this.metricsService.getAggregatedMetrics(
  ...
  private calculateSuccessRate(): number {
    const successCount = this.metricsService.getMetrics('analysis.requests', {
      status: 'success',
    }).length;
